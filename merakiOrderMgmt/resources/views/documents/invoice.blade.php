    ...
    <section class="content">
      ...
      <div class="row invoice-info">
        ...
        <div class="col-sm-4 invoice-col">
          Invoice To
          <address>
            <strong>Mr.{{ $enquiry->name }}</strong><br>
            {{ $enquiry->designation }}<br>
        ...
        <div class="col-sm-4 invoice-col">
          Ship To
          <address>
            <strong>Mr.{{ $order->contactPersonAtShipment }}</strong><br>
            @php
              $addrArray = explode(",",$order->shipmentAddress);
            @endphp
            @for($v=0; $v<count($addrArray); $v++)
              {{ $addrArray[$v] }}<br>
            @endfor
          </address>
        </div>
        <!-- /.col -->
        <div class="col-sm-4 invoice-col">
          <b>Date: </b> {{ date('d-M-Y', strtotime($order->invoiceDate)) }} <br>
          <b>Due Date: </b> {{ date('d-M-Y', strtotime($order->invoiceDueDate)) }} <br>
          <b>GSTIN: </b> {{ $order->GSTIN }} <br>
          <b>Reference No: </b> {{ $order->documentNumber }} <br>
          <b>PO: </b> {{ $order->purchaseOrderNumber }} <br>
